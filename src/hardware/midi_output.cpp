...
bool MidiOutput::setFunctionKeyColor(int index, BYTE color)
{
    const int led = launchpadFunctionKey(index);
    if (!device_ || led == 0) {
        return false;
...
bool MidiOutput::setLedColor(int x, int y, BYTE color)
{
...
bool MidiOutput::setTopFunctionKeyColor(int index, BYTE color)
{
    const int led = launchpadTopFunctionKey(index);
    if (!device_ || led == 0) {
        return false;
